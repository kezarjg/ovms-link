...
function callbackVehicleOff() {
  ...
  snap.is_dcfc = false;
  snap[C.RESYNC_KEY] = true;   // send full: the forced fields usually match the last
                               // live sample, and delta encoding would strip them all
...
/**
 * Startup wiring: apply vehicle overrides and wait for valid GPS time. (CA-root
 * installation lives in the separate abrp-certs plugin, not here.)
 */
function startup() {
