...
/**
 * Deferred startup work, run on the first ticker.1 rather than synchronously at
 * module load. Certs.bootstrap() nests a require('plugin/abrp/certdata'); running
 * that during the require of the plugin module element (already deep in OVMS's
 * plugin loader) overflowed the DukTape load stack -> WDT reboot loop. On a ticker
 * the callback runs from the event loop with stack headroom, so the nested require
 * is safe. bootstrap() self-gates (config version stamp + inProgress), so calling
 * it on each tick until GPS time is valid is idempotent; checkTime() unsubscribes
 * ticker.1 once time is valid.
 */
...
/**
 * Startup wiring: apply vehicle overrides (cheap, no nested require) and subscribe
 * the deferred startup + config-reload handlers. The heavy/nested-require work is
 * deferred to deferredStartup so nothing runs on the synchronous module-load stack.
 */
function startup() {
  Met.overrideMetricMap()
  subscribe('ticker.1', deferredStartup)
