...
test('changedVsLastQueued: sub-threshold charging-power jitter is not a change (deadband)', () => {
  const Q = loadQueue()
  // Charging at ~-44 kW; jitter under CHARGE_POWER_DELTA_KW (1 kW) must NOT queue —
  // this is the DC-fast-charge point-flood fix, ported from 2.3.0.
  Q.setLastQueued({ utc: 10, power: -44.0, soc: 60, is_charging: true })
  assert.strictEqual(Q.changedVsLastQueued({ utc: 11, power: -44.6, soc: 60, is_charging: true }), false) // 0.6 kW jitter
  assert.strictEqual(Q.changedVsLastQueued({ utc: 11, power: -45.2, soc: 60, is_charging: true }), true)  // 1.2 kW move
  assert.strictEqual(Q.changedVsLastQueued({ utc: 11, power: -44.6, soc: 61, is_charging: true }), true)  // SoC still queues
  // Not charging: the deadband must not apply — a 0.1 kW move is still a change (drive path unchanged).
  Q.setLastQueued({ utc: 10, power: 5.1, soc: 50, is_charging: false })
  assert.strictEqual(Q.changedVsLastQueued({ utc: 11, power: 5.2, soc: 50, is_charging: false }), true)
})

test('setChargePowerDeltaKw overrides the charging deadband width', () => {
  const Q = loadQueue()
  Q.setChargePowerDeltaKw(2) // widen the deadband to 2 kW
  Q.setLastQueued({ utc: 10, power: -44.0, soc: 60, is_charging: true })
  assert.strictEqual(Q.changedVsLastQueued({ utc: 11, power: -45.5, soc: 60, is_charging: true }), false) // 1.5 < 2
  assert.strictEqual(Q.changedVsLastQueued({ utc: 11, power: -46.5, soc: 60, is_charging: true }), true)  // 2.5 >= 2
})

test('setChargePowerDeltaKw(0) disables the deadband (any charging power move queues)', () => {
  const Q = loadQueue()
  Q.setChargePowerDeltaKw(0)
  Q.setLastQueued({ utc: 10, power: -44.0, soc: 60, is_charging: true })
  assert.strictEqual(Q.changedVsLastQueued({ utc: 11, power: -44.1, soc: 60, is_charging: true }), true)
})
...
test('setHeartbeatInterval overrides the heartbeat used by sample()', () => {
  const present = { 'm.time.utc': 1000, 'v.b.soc': 50 }
  const mono = { t: 0 }
  const Q = loadQueueWithClock(present, mono)
  Q.setSampleInterval(3)
  Q.getQueue().length = 0
  mono.t = 0; Q.enqueue(Q.roundTelemetry({ utc: 1000, soc: 50 }))
  Q.setHeartbeatInterval(50)          // shorter than the 160 default
  mono.t = 60; Q.sample()             // unchanged, but 60 >= 50 -> heartbeat forces a point
  assert.strictEqual(Q.getQueue().length, 2)
})

test('setHeartbeatInterval(0) disables the heartbeat', () => {
  const present = { 'm.time.utc': 1000, 'v.b.soc': 50 }
  const mono = { t: 0 }
  const Q = loadQueueWithClock(present, mono)
  Q.setSampleInterval(3)
  Q.getQueue().length = 0
  mono.t = 0; Q.enqueue(Q.roundTelemetry({ utc: 1000, soc: 50 }))
  Q.setHeartbeatInterval(0)
  mono.t = 100000; Q.sample()         // huge elapsed, unchanged -> still no forced point
  assert.strictEqual(Q.getQueue().length, 1)
})
