 ...
 * changed from the previous point. Points flagged with RESYNC_KEY are also sent
 * full — a session bookend asserts a complete state (parked/not charging) that
 * ABRP must see even when it happens to match the last live sample, which is the
 * normal case (gear reaches P a sample before the ignition-off event fires).
 * Does not mutate the input points.
 */
function deltaEncode(batch) {
  ...
  for (var i = 0; i < batch.length; i++) {
    var p = batch[i]
    if (prev === null || p[C.RESYNC_KEY]) {
      out.push(stripResync(p))
    } else {
      var d = { utc: p.utc }
      for (var k in p) {
        if (k === 'utc' || k === C.RESYNC_KEY) { continue }
        if (p[k] !== prev[k]) { d[k] = p[k] }
      }
      out.push(d)
    }
    prev = p
...
/**
 * Copies a point without the internal resync marker. The marker is queue-side
 * bookkeeping and must never reach ABRP; the queued point keeps it, so a batch
 * held back by a failed send is still re-sent in full.
 */
function stripResync(p) {
  var out = clone(p)
  delete out[C.RESYNC_KEY]
  return out
}
