...
function snapshot() {
  ...
  return {
    ...
    telemetry: tele,
    // Effective config (defaults applied by Cfg when unset) so the config web
    // page can auto-populate its fields — it can't read defaults from OVMS
    // `config list`, which only reports explicitly-set keys.
    config: {
      sample_interval: Cfg.sampleInterval(),
      send_interval: Cfg.sendInterval(),
      heartbeat_interval: Cfg.heartbeatInterval(),
      charge_power_delta_kw: Cfg.chargePowerDeltaKw(),
    },
  };
}
