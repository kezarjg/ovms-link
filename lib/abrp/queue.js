 ...
 *
 * While charging, the fields named in CHARGE_DEADBAND (power/current/voltage) get a
 * per-field deadband: sub-threshold jitter is not treated as a change, so noisy DC
 * fast-charge readings (which can swing kW/A/V every sample) no longer force a point
 * every second. SOC steps, state flips, and any other field still queue normally, and
 * each deadband measures against the last *queued* value, so a slow drift still
 * accumulates to a point. Driving keeps full per-ROUNDING resolution (deadband is
 * charge-only).
 */
function changedVsLastQueued(snap) {
  var deadbands = snap.is_charging === true ? C.CHARGE_DEADBAND : null
  for (var k in snap) {
    if (k === 'utc') { continue }
    if (
      deadbands && deadbands[k] > 0 &&
      typeof snap[k] === 'number' && typeof lastQueuedTelemetry[k] === 'number'
    ) {
      if (Math.abs(snap[k] - lastQueuedTelemetry[k]) >= deadbands[k]) { return true }
      continue
    }
