...
module.exports = {
  ...
  CHARGE_DEADBAND: { power: 1, current: 5, voltage: 2 }, // while charging, ignore sub-threshold jitter on these fields (kW / A / V) so DC fast-charge noise doesn't force a point every sample; omit a field or set it to 0 to keep that field's per-ROUNDING resolution
  SEND_INTERVAL_DEFAULT: 30, // seconds between bulk flushes; overridden by usr abrp.send_interval (10-60)
  BW_HTTP_OVERHEAD: 120, // bytes; fixed per-request header allowance for the bandwidth counter (app-layer estimate; excludes TLS/TCP framing)
  ROUNDING: {
