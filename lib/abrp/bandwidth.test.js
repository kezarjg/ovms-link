...
test('snapshot reports elapsed_s from the monotonic clock since reset', () => {
  global.OvmsMetrics = { Value: () => 1000 }
  Bw.reset() // baselines at 1000
  global.OvmsMetrics = { Value: () => 1042 }
  assert.strictEqual(Bw.snapshot().elapsed_s, 42)
  delete global.OvmsMetrics
})

test('elapsed_s is never negative if the clock is unavailable', () => {
  delete global.OvmsMetrics
  Bw.reset()
  assert.strictEqual(Bw.snapshot().elapsed_s, 0)
})
