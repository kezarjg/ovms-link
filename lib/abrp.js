...
const BW_HTTP_OVERHEAD = 120 // bytes; fixed per-request header allowance for the bandwidth counter (app-layer estimate; excludes TLS/TCP framing)
...
var subscriptions = {};
var isSending = false
// Bandwidth accounting (Tier 1): RAM-only app-layer byte/request counters per
// phase. Resets on reboot; read via info().
var bandwidthStats = {
  driving: { up: 0, down: 0, reqs: 0 },
  charging: { up: 0, down: 0, reqs: 0 },
  idle: { up: 0, down: 0, reqs: 0 },
}
// Monotonic baseline (seconds-since-boot) the bandwidth window is timed from.
function readMonotonic() {
  return (typeof OvmsMetrics !== 'undefined' && OvmsMetrics.Value)
    ? (OvmsMetrics.Value('m.monotonic') || 0)
    : 0
}
var bwStartMono = readMonotonic()
...
/**
 * Records one completed request's app-layer bytes against the phase of `point`.
 * Call only for requests that received a response — an offline modem sends
 * nothing and consumes no bandwidth.
 */
function recordBandwidth(point, up, down) {
  var b = bandwidthStats[classifyPhase(point)]
  b.up += up
  b.down += down
  b.reqs += 1
}
...
/**
 * Returns a fresh copy of the per-phase counters plus a computed total. Safe to
 * retain: later recordBandwidth() calls do not mutate a returned snapshot.
 */
function bandwidthSnapshot() {
  var out = { total: { up: 0, down: 0, reqs: 0 } }
  var keys = ['driving', 'charging', 'idle']
  for (var i = 0; i < keys.length; i++) {
    var k = keys[i]
    out[k] = { up: bandwidthStats[k].up, down: bandwidthStats[k].down, reqs: bandwidthStats[k].reqs }
    out.total.up += bandwidthStats[k].up
    out.total.down += bandwidthStats[k].down
    out.total.reqs += bandwidthStats[k].reqs
  }
  out.elapsed_s = Math.max(0, readMonotonic() - bwStartMono)
  return out
}

/**
 * Zeroes all bandwidth counters and re-baselines the elapsed-time window to now.
 */
...
function sendTelemetry(telemetry) {
  ...
  HTTP.Request({
    ...
    timeout: 5000,
      ...
      recordBandwidth(telemetry, url.length + BW_HTTP_OVERHEAD, (response.body || '').length)
  ...
  try {
    HTTP.Request({
      ...
      timeout: 8000, // must complete within ticker.10
        ...
        recordBandwidth(batch[batch.length - 1], url.length + body.length + BW_HTTP_OVERHEAD, (response.body || '').length)
        if (response.statusCode === 200 && isApiOk(response.body)) {
...
function callbackVehicleOff() {
  ...
  // Persist the session bandwidth tally to the log (DEBUG-gated) so the RAM-only
  // counters survive a later sleep/reboot in the rotated log files.
  Logger.debug('ABRP bandwidth (cumulative since boot)', bandwidthSnapshot());
}
...
function logBandwidth() {
  var bw = bandwidthSnapshot();
  function kb(n) { return (n / 1024).toFixed(1) + ' KB'; }
  function row(label, b) {
    Logger.log('  ' + label + ' up ' + kb(b.up) + '  down ' + kb(b.down) + '  ' + b.reqs + ' req');
  }
  Logger.log('ABRP bandwidth (' + (bw.elapsed_s / 3600).toFixed(1) + ' h since boot, app-layer est.):');
  row('driving ', bw.driving);
  row('charging', bw.charging);
  row('idle    ', bw.idle);
  row('total   ', bw.total);
  if (bw.elapsed_s > 0) {
    var perDay = (bw.total.up + bw.total.down) / 1024 / bw.elapsed_s * 86400;
    Logger.log('  rate     ' + perDay.toFixed(0) + ' KB/day (up+down at this usage mix)');
  }
}
