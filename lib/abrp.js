...
const METRIC_POLL_STALE_CONNECTION = (3 * 60) - 20 - 60 // 3 minutes for OVMS API Key
const MAXIMUM_PER_TRANSMIT = 10 // Maximum number of telemetry for every bulk upload
...
function processTelemetryQueue() {
  if (sendingTelemetry) {
    Logger.debug('Telemetry is already being sent. Skipping this processing.');
    return;  // Exit the function if telemetry is already being sent
  }

  if (telemetryToSend.length > 0) {
    Logger.debug('Processing queue, data in queue:', telemetryToSend.length);
...
function manageVehicleStateEvents(subscribe) {
  ...
  if (subscribe) {
    ...
    PubSub.subscribe('ticker.10', processTelemetryQueue)
