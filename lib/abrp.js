...
function calculateMaxElapsedDuration(telemetry) {
  ...
  if (!telemetry.is_parked || telemetry.is_dcfc) {
    // TODO: I think there is a bug here... Doesn't detect when fast charging. Might be an OVMS problem with the Solterra module.
...
function sendBulkTelemetry() {
  if (isSendingTelemetry) {
    Logger.warn('sendBulkTelemetry called while a previous instance is still running. Exiting.');
