...
function callbackVehicleOn() {
  ...
  tickerToken = PubSub.subscribe('ticker.1', queueTelemetryIfNecessary);
}
...
function callbackVehicleOff() {
  ...
  PubSub.unsubscribe(tickerToken); // Pass the token to unsubscribe
...
function manageVehicleStateEvents(subscribe) {
  ...
  if (subscribe) {
    ...
    PubSub.subscribe('ticker.10', sendBulkTelemetry)
