...
var Console = logger()
var MIN_VALID_TIME = 946684800; // Unix timestamp for Jan 1, 2000
...
function queueTelemetryIfNecessary() {
  ...
  // Check if the GPS time (utc) is valid, assuming a valid time should be after Jan 1, 2000 (946684800 in Unix timestamp)
  if (currentTelemetry.utc <= MIN_VALID_TIME) {
    Console.debug('Invalid GPS time, skipping telemetry processing.');
    return; // Skip further processing if time is invalid
  }
...
function manageVehicleStateEvents(subscribe) {
  ...
  if (subscribe) {
    ...
    PubSub.subscribe('ticker60', processTelemetryQueue)
