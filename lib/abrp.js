...
var METRIC_POLL_STALE_CONNECTION = (3 * 60) - 20 - 60 // 3 minutes for OVMS API Key
var MAXIMUM_PER_TRANSMIT = 10 // Maximum number of telemetry for every bulk upload
...
function createBulkPost() {
  ...
  return {
    data: [
      {
        token: user_token,
        tlm_list: telemetryToSend.slice(0, MAXIMUM_PER_TRANSMIT)  // Take only the first 10 elements
      }
    ]
  };
}
...
function sendBulkTelemetry() {
  Console.debug('Sending bulk telemetry to ABRP');
  var url =
  ...
  HTTP.Request({
    ...
    post: JSON.stringify(bulkPost),
      ...
      if (count === MAXIMUM_PER_TRANSMIT) {
        // There may be telemetry left in the queue
...
function processTelemetryQueue() {
  if (sendingTelemetry) {
    Console.debug('Telemetry is already being sent. Skipping this processing.');
    return;  // Exit the function if telemetry is already being sent
  }
...
function calculateMaxElapsedDuration(telemetry) {
  ...
  if (telemetry.speed > MIN_CALIBRATION_SPEED) {
    Console.debug('Speed greater than minimum calibration speed');
    return METRIC_POLL_RATE_DRIVING;
  }

  if (!telemetry.is_parked || telemetry.is_dcfc) {
    Console.debug('Driving or DC fast charging');
    return METRIC_POLL_STALE_CONNECTION;
  }

  if (telemetry.is_charging) {
    Console.debug('Standard charging');
...
function manageVehicleStateEvents(subscribe) {
  ...
  if (subscribe) {
    ...
    PubSub.subscribe('ticker.60', processTelemetryQueue)
    ...
    if (OvmsMetrics.Value('v.e.on')) {
      ...
      Console.debug('Vehicle is ON or charging');
      callbackVehicleOn();  
    } else {
 ...
 */
function checkTime() {
  var minValidTime = 946684800; // Unix timestamp for Jan 1, 2000
  if (OvmsMetrics.Value('m.time.utc') > minValidTime) {
    isTimeValid = true;  // Mark the time as valid
    Console.debug('GPS time is valid, unsubscribing from ticker.1');
    
    // Unsubscribe from the ticker.1 event once the time is valid
    PubSub.unsubscribe('ticker.1', checkTime);
    
    // Proceed with startup
    send(true);
  } else {
    Console.debug('Invalid GPS time, skipping telemetry processing.');
  }
}
...
function send(shouldSend) {
  ...
  if (!isTimeValid) {
    Console.error('Cannot send data: GPS time is invalid.');
    return;
  }
...
// Main Logic
overrideMetricMap();
PubSub.subscribe('ticker.1', checkTime);
