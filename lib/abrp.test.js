...
describe('config.changed live-reload (bundle)', () => {
  ...
  it('defers cert bootstrap off the load stack until the first ticker.1', () => {
    // Regression (Stage-2 plugin crash): as a plugin module element the bundle is
    // require()'d from deep in OVMS's plugin loader; running Certs.bootstrap() —
    // which nests a require('plugin/abrp/certdata') — synchronously at load then
    // overflowed the DukTape load stack -> WDT reboot loop. Bootstrap must run on
    // the first ticker.1 (event-loop stack, headroom), not during require().
    const subs = {}
    let nextTok = 1
    const pubsub = {
    ...
    const fire = (topic) => { if (subs[topic]) Object.keys(subs[topic]).forEach((t) => subs[topic][t](topic)) }
    let loaderCalls = 0
    try {
      const abrp = loadAbrp({
        // Certs.bootstrap() only reaches the cert-data loader when these host
        // APIs are present; without them it early-returns (off-device hand-install).
        VFS: { Save: () => {} },
        OvmsCommand: { Exec: () => {} },
        OvmsConfig: { Get: () => '0', Set: () => {}, GetValues: () => ({ user_token: 'T' }) },
        OvmsMetrics: { Value: (k) => (k === 'm.time.utc' ? 1700000000 : 0), HasValue: () => false, GetValues: () => ({}) },
        PubSub: pubsub,
      })
      abrp.__test.setCertDataLoader(() => { loaderCalls++; return [] })
      assert.strictEqual(loaderCalls, 0)   // NOT run synchronously during require()
      fire('ticker.1')
      assert.strictEqual(loaderCalls, 1)   // run on the first ticker.1
    } finally {
      delete global.VFS
      delete global.OvmsCommand
    }
  })
})
