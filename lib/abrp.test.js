...
describe('charge deadband (bundle)', () => {
  // A coherent DC-fast-charge baseline: -99 kW, -230 A, 340 V, soc 20.
  function chargingBaseline() {
    return {
      utc: 100, soc: 20, power: -99.0, current: -230, voltage: 340,
      is_charging: true, is_dcfc: true, is_parked: true,
    }
  }

  it('ignores sub-deadband power jitter while charging, but not SOC or large swings', () => {
    const abrp = loadAbrp()
    abrp.__test.setLastQueued(chargingBaseline())

    // Sub-deadband jitter (0.6 kW) while charging -> NOT a change.
    assert.strictEqual(
      abrp.__test.changedVsLastQueued(Object.assign(chargingBaseline(), { utc: 101, power: -99.6 })),
      false,
    )
    // Power swing >= deadband (1.2 kW) while charging -> change.
    assert.strictEqual(
      abrp.__test.changedVsLastQueued(Object.assign(chargingBaseline(), { utc: 102, power: -100.2 })),
      true,
    )
    // SOC step always queues, even with tiny power jitter.
    assert.strictEqual(
      abrp.__test.changedVsLastQueued(Object.assign(chargingBaseline(), { utc: 103, soc: 21, power: -99.1 })),
      true,
    )
  })

  it('ignores sub-deadband current jitter while charging', () => {
    const abrp = loadAbrp()
    abrp.__test.setLastQueued(chargingBaseline())
    // 3 A jitter (< 5 A) -> NOT a change.
    assert.strictEqual(
      abrp.__test.changedVsLastQueued(Object.assign(chargingBaseline(), { utc: 101, current: -233 })),
      false,
    )
    // 8 A swing (>= 5 A) -> change.
    assert.strictEqual(
      abrp.__test.changedVsLastQueued(Object.assign(chargingBaseline(), { utc: 102, current: -238 })),
      true,
    )
  })

  it('ignores sub-deadband voltage jitter while charging', () => {
    const abrp = loadAbrp()
    abrp.__test.setLastQueued(chargingBaseline())
    // 1 V step (< 2 V) -> NOT a change.
    assert.strictEqual(
      abrp.__test.changedVsLastQueued(Object.assign(chargingBaseline(), { utc: 101, voltage: 341 })),
      false,
    )
    // 3 V swing (>= 2 V) -> change.
    assert.strictEqual(
      abrp.__test.changedVsLastQueued(Object.assign(chargingBaseline(), { utc: 102, voltage: 343 })),
      true,
    )
  })

  it('does not apply the deadband while driving (full resolution preserved)', () => {
    const abrp = loadAbrp()
    abrp.__test.setLastQueued({
      utc: 100, soc: 50, power: 10.0, current: 20, voltage: 350,
      is_charging: false, is_dcfc: false, is_parked: false,
    })
    // Sub-charge-deadband changes while driving still count (deadband is charge-only).
    assert.strictEqual(
      abrp.__test.changedVsLastQueued({
        utc: 101, soc: 50, power: 10.6, current: 20, voltage: 350,
        is_charging: false, is_dcfc: false, is_parked: false,
      }),
      true,
    )
    assert.strictEqual(
      abrp.__test.changedVsLastQueued({
        utc: 102, soc: 50, power: 10.0, current: 21, voltage: 350,
        is_charging: false, is_dcfc: false, is_parked: false,
      }),
      true,
    )
...
describe('charge/drive overlap state machine (bundle)', () => {
  // Boots the bundle through the real checkTime -> send(true) flow so the
  ...
  function boot() {
    let nextToken = 1
    const subs = {} // token -> {topic, cb}, mirrors PubSub's token contract
    const ps = {
      subscribe: (topic, cb) => { const t = 'tok' + nextToken++; subs[t] = { topic, cb }; return t },
      unsubscribe: (t) => { delete subs[t] },
    }
    const fire = (topic) => {
      Object.keys(subs).forEach((k) => { if (subs[k] && subs[k].topic === topic) subs[k].cb(topic) })
    }
    const count = (topic) => Object.keys(subs).filter((k) => subs[k].topic === topic).length
    const present = {
  ...
  it('unplugging while the vehicle is on keeps the sampler until vehicle.off', () => {
    ...
    assert.strictEqual(q[q.length - 1].is_parked, true) // forced parked bookend
  })
...
describe('bandwidth counter', () => {
  function setup() {
    ...
    abrp.__test.setSendInterval(0)
    abrp.__test.resetBandwidth()
    const q = abrp.__test.getQueue()
    q.length = 0
    return { abrp, requests, q }
  }

  it('records a flush against the driving bucket with up/down/reqs', () => {
    const { abrp, requests, q } = setup()
    q.push({ utc: 1, is_parked: false })
    abrp.__test.sendBulkTelemetry()
    const body = '{"status":"ok"}'
    requests[0].done({ statusCode: 200, body })
    const bw = abrp.__test.bandwidth()
    assert.strictEqual(bw.driving.reqs, 1)
    assert.ok(bw.driving.up > 0)
    assert.strictEqual(bw.driving.down, body.length)
    assert.strictEqual(bw.total.reqs, 1)
  })

  it('counts a 200-with-status:error flush — bytes still crossed the wire', () => {
    const { abrp, requests, q } = setup()
    q.push({ utc: 1, is_charging: true, is_parked: true })
    abrp.__test.sendBulkTelemetry()
    requests[0].done({ statusCode: 200, body: '{"status":"error"}' })
    assert.strictEqual(abrp.__test.bandwidth().charging.reqs, 1)
  })

  it('does not count a failed (unsent) request', () => {
    const { abrp, requests, q } = setup()
    q.push({ utc: 1, is_parked: false })
    abrp.__test.sendBulkTelemetry()
    requests[0].fail('network unavailable')
    assert.strictEqual(abrp.__test.bandwidth().total.reqs, 0)
  })

  it('reports elapsed_s since reset from the monotonic clock', () => {
    let mono = 1000
    const abrp = loadAbrp({
      HTTP: { Request: () => {} },
      OvmsMetrics: { Value: () => mono, HasValue: () => false, GetValues: () => ({}) },
    })
    abrp.__test.resetBandwidth()
    mono = 1042
    assert.strictEqual(abrp.__test.bandwidth().elapsed_s, 42)
  })

  it('logs the bandwidth snapshot on the vehicle-off bookend when DEBUG', () => {
    const lines = []
    const abrp = loadAbrp({
      HTTP: { Request: () => {} },
      PubSub: { subscribe: () => 1, unsubscribe: () => {} },
    ...
    abrp.__test.resetBandwidth()
    const saved = global.print
    global.print = (s) => { lines.push(String(s)) }
    try {
      abrp.__test.callbackVehicleOff()
    } finally {
      global.print = saved
    ...
    assert.ok(
      lines.some((l) => l.indexOf('ABRP bandwidth') !== -1),
      'expected a bandwidth snapshot line in the log'
    )
  })
})
