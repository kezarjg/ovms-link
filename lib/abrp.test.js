...
describe('bandwidth counter', () => {
  ...
  test('records a flush against the driving bucket with up/down/reqs', () => {
    const { abrp, requests, q } = setup()
    ...
    expect(bw.driving.reqs).toBe(1)
    expect(bw.driving.up).toBeGreaterThan(0)
    expect(bw.driving.down).toBe(body.length)
    expect(bw.total.reqs).toBe(1)
  })

  test('counts a 200-with-status:error flush (bytes still crossed the wire)', () => {
    const { abrp, requests, q } = setup()
    q.push({ utc: 1, is_charging: true, is_parked: true })
    abrp.__test.sendBulkTelemetry()
    requests[0].done({ statusCode: 200, body: '{"status":"error"}' })
    expect(abrp.__test.bandwidth().charging.reqs).toBe(1)
  })

  test('does not count a failed (unsent) request', () => {
    const { abrp, requests, q } = setup()
    q.push({ utc: 1, is_parked: false })
    abrp.__test.sendBulkTelemetry()
    requests[0].fail('network unavailable')
    expect(abrp.__test.bandwidth().total.reqs).toBe(0)
  })
})

describe('bandwidth bookend logging', () => {
  test('logs the bandwidth snapshot on the vehicle-off bookend when DEBUG', () => {
    const lines = []
    const abrp = loadAbrp({
      HTTP: { Request: () => {} },
    ...
    expect(lines.some((l) => l.includes('ABRP bandwidth'))).toBe(true)
  })
})

describe('bandwidth elapsed time', () => {
  test('snapshot reports elapsed_s from the monotonic clock since reset', () => {
    let mono = 1000
    const abrp = loadAbrp({
      HTTP: { Request: () => {} },
    ...
    abrp.__test.resetBandwidth()
    mono = 1050
    expect(abrp.__test.bandwidth().elapsed_s).toBe(50)
  })
})
