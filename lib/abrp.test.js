...
function loadAbrp(globals) {
  ...
  ;['OvmsConfig', 'OvmsMetrics', 'PubSub', 'HTTP', 'OvmsNotify', 'VFS', 'OvmsCommand'].forEach((k) => {
    delete global[k]
  })
...
describe('delta encoding (createBulkPost)', () => {
  ...
  it('re-sends a resync point in full even when nothing changed', () => {
    const abrp = loadAbrp({ OvmsConfig: { GetValues: () => ({ user_token: 'T' }) } })
    const post = abrp.createBulkPost([
      { utc: 1, soc: 50, power: 3, is_parked: true },
      { utc: 2, soc: 50, power: 3, is_parked: true, __resync: true },
    ])
    const list = post.data[0].tlm_list
    // Identical to the previous point, but forced full — and the marker stays off the wire.
    assert.deepStrictEqual(list[1], { utc: 2, soc: 50, power: 3, is_parked: true })
  })

  it('leaves the resync marker on the queued point so a retried batch stays full', () => {
    const abrp = loadAbrp({ OvmsConfig: { GetValues: () => ({ user_token: 'T' }) } })
    const batch = [{ utc: 1, soc: 50 }, { utc: 2, soc: 50, __resync: true }]
    abrp.createBulkPost(batch)
    assert.strictEqual(batch[1].__resync, true) // a failed send keeps the batch queued
  })
})

describe('vehicle-off bookend on the wire', () => {
  // Regression, on-device log 2026-07-12 14:08 (Solterra): gear -> P flips is_parked
  // true and speed to 0 one sample BEFORE the ignition-off event, so every field the
  // bookend forces already matched the last live sample. The delta encoder stripped
  // them all and the final point ABRP received for the trip was {utc, power: 0} — it
  // never said the car was parked, so ABRP left the trip open.
  it('carries the forced parked state even when unchanged vs the last live sample', () => {
    const present = { 'm.time.utc': 2000, 'v.b.soc': 60, 'v.p.speed': 0, 'v.b.power': 0, 'v.e.parktime': 120 }
    const mono = { t: 0 }
    const abrp = loadAbrp({
    ...
    // Last live sample: already stopped and parked, ignition still on.
    abrp.__test.enqueue(abrp.__test.roundTelemetry(abrp.__test.createTelemetry()))
    present['m.time.utc'] = 2002
    abrp.__test.callbackVehicleOff()

    const wire = abrp.createBulkPost(abrp.__test.getQueue()).data[0].tlm_list
    const last = wire[wire.length - 1]
    assert.strictEqual(last.is_parked, true)
    assert.strictEqual(last.speed, 0)
    assert.strictEqual(last.power, 0)
    assert.strictEqual(last.is_charging, false)
    assert.strictEqual(last.is_dcfc, false)
    assert.ok(!('__resync' in last), 'internal marker must not reach ABRP')
  })
})
...
describe('config.changed live-reload (bundle)', () => {
  ...
  it('applyIntervals live-reloads heartbeat_interval and charge_power_delta_kw', () => {
    const abrp = loadAbrp({
      OvmsConfig: { GetValues: () => ({ user_token: 'T', heartbeat_interval: '300', charge_power_delta_kw: '2.5' }) },
      OvmsMetrics: { Value: () => 0, HasValue: () => false, GetValues: () => ({}) },
      PubSub: { subscribe: () => 1, unsubscribe: () => {} },
    })
    abrp.__test.applyIntervals()
    assert.strictEqual(abrp.__test.getHeartbeatInterval(), 300)
    assert.strictEqual(abrp.__test.getChargePowerDeltaKw(), 2.5)
  })
...
describe('abrp web seam (snapshot / meUrl)', () => {
  const C = require('../lib/abrp/constants')

  it('snapshot() returns a plain operational-state object (no identity)', () => {
    const abrp = loadAbrp({
    ...
    assert.ok(!('identity' in s)) // identity is owned by the abrp-web plugin
  })

  it('snapshot() includes key telemetry fields when sources have values', () => {
    const abrp = loadAbrp({
      OvmsConfig: { GetValues: () => ({ user_token: 'TOK' }) },
  ...
  it('snapshot() exposes the effective config (defaults applied when unset)', () => {
    // The config web page reads these to auto-populate its fields. With nothing
    // set in OVMS config, it must report the effective *defaults*, not blanks.
    const abrp = loadAbrp({
      OvmsConfig: { GetValues: () => ({ user_token: 'TOK' }) }, // no interval/deadband keys
      OvmsMetrics: { Value: () => 0, HasValue: () => false, GetValues: () => ({}) },
    })
    const c = abrp.snapshot().config
    assert.strictEqual(c.sample_interval, C.SAMPLE_INTERVAL_DEFAULT)
    assert.strictEqual(c.send_interval, C.SEND_INTERVAL_DEFAULT)
    assert.strictEqual(c.heartbeat_interval, C.HEARTBEAT_INTERVAL)
    assert.strictEqual(c.charge_power_delta_kw, C.CHARGE_POWER_DELTA_KW)
  })

  it('snapshot() config reflects explicitly-set values', () => {
    const abrp = loadAbrp({
      OvmsConfig: { GetValues: () => ({ user_token: 'TOK', sample_interval: '2', send_interval: '45' }) },
      OvmsMetrics: { Value: () => 0, HasValue: () => false, GetValues: () => ({}) },
    })
    const c = abrp.snapshot().config
    assert.strictEqual(c.sample_interval, 2)
    assert.strictEqual(c.send_interval, 45)
  })

  it('meUrl() builds the oauth/me URL for the current token, or "" when unset', () => {
    const withTok = loadAbrp({
      OvmsConfig: { GetValues: () => ({ user_token: 'a b' }) },
      OvmsMetrics: { Value: () => 0, HasValue: () => false, GetValues: () => ({}) },
    })
    const url = withTok.meUrl()
    assert.ok(url.indexOf('https://api.iternio.com/1/oauth/me?') === 0)
    assert.ok(url.indexOf('access_token=a%20b') !== -1)

    const noTok = loadAbrp({
      OvmsConfig: { GetValues: () => ({}) },
      OvmsMetrics: { Value: () => 0, HasValue: () => false, GetValues: () => ({}) },
    })
    assert.strictEqual(noTok.meUrl(), '')
  })
})
...
describe('charge/drive overlap state machine (bundle)', () => {
  // PubSub stub that tracks live subscriptions per topic so a test can fire an
  // event and assert how many samplers/handlers are wired.
  function makePubSub() {
    const subs = {}         // topic -> { token: cb }
    const tokenTopic = {}   // token -> topic
    let next = 1
    const pubsub = {
      subscribe(topic, cb) {
        const tok = next++
        ...
        subs[topic][tok] = cb
        tokenTopic[tok] = topic
        return tok
      },
      unsubscribe(tok) {
        const topic = tokenTopic[tok]
        if (topic && subs[topic]) delete subs[topic][tok]
      },
    }
    return {
      pubsub,
      fire(topic) { if (subs[topic]) Object.keys(subs[topic]).forEach((t) => subs[topic][t](topic)) },
      count(topic) { return subs[topic] ? Object.keys(subs[topic]).length : 0 },
    }
  }

  // Boot through the real checkTime -> send(true) flow, then drive the machine by
  // firing PubSub events while mutating metric levels. The sampler must track the
  // LEVEL (v.e.on || v.c.charging), not raw event edges.
  function boot(initialLevels) {
    const ps = makePubSub()
    const present = Object.assign({
      'm.time.utc': 1700000000, 'v.b.soc': 50, 'v.e.parktime': 100,
      'v.e.on': false, 'v.c.charging': false, 'm.monotonic': 0,
    }, initialLevels || {})
    const abrp = loadAbrp({
      OvmsConfig: { GetValues: () => ({ user_token: 'T' }) },
      OvmsNotify: { Raise: () => {} },
    ...
    ps.fire('ticker.1') // valid GPS: checkTime unsubscribes itself and runs send(true)
    return { abrp, present, fire: ps.fire, count: ps.count }
  }

  it('vehicle.on while already charging does not double-subscribe the sampler', () => {
    const { abrp, present, fire, count } = boot()
    abrp.__test.getQueue().length = 0
    present['v.c.charging'] = true
    fire('vehicle.charge.start')
    assert.strictEqual(count('ticker.1'), 1)             // sampler on
    present['v.e.on'] = true
    fire('vehicle.on')                                    // driver gets in while still plugged
    assert.strictEqual(count('ticker.1'), 1)             // still exactly one sampler
    assert.strictEqual(abrp.__test.getQueue().length, 1) // a single session-start bookend
  })

  it('unplugging while the vehicle is on keeps the sampler until vehicle.off', () => {
    const { abrp, present, fire, count } = boot()
    abrp.__test.getQueue().length = 0
    present['v.c.charging'] = true
    fire('vehicle.charge.start')
    ...
    assert.strictEqual(count('ticker.1'), 1)             // sampler must survive the unplug
    present['v.e.on'] = false
    fire('vehicle.off')
    assert.strictEqual(count('ticker.1'), 0)             // session over, sampler stops
    assert.strictEqual(abrp.__test.getQueue().length, 2) // exactly one start + one off bookend
  })

  it('send(0)/send(1) cycles do not stack ticker.10 / vehicle.type.set subscriptions', () => {
    const { abrp, count } = boot()
    abrp.send(0)
    abrp.send(1)
    assert.strictEqual(count('ticker.10'), 1)
    assert.strictEqual(count('vehicle.type.set'), 1)
  })

  it('booting while charging (ignition off) starts the session', () => {
    const { count } = boot({ 'v.c.charging': true })
    assert.strictEqual(count('ticker.1'), 1)             // sampler subscribed at startup
  })
})
